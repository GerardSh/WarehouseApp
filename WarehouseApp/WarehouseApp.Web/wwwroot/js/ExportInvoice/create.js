...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.addProduct = function () {
        ...
        <tr class="product-group">
             <td>
                <input type="text"
                       name="ExportedProducts[${newIndex}].ImportInvoiceNumber"
                       class="form-control bg-dark text-white"
                       placeholder="Import Invoice Number"
                       data-val="true"
                       data-val-required="ImportInvoiceNumber is required."
                       data-val-minlength="ImportInvoiceNumber must be at least 3 characters."
                       data-val-minlength-min="3"
                       data-val-maxlength="ImportInvoiceNumber cannot be longer than 255 characters."
                       data-val-maxlength-max="255" />
                <span class="text-danger field-validation-valid"
                      data-valmsg-for="ExportedProducts[${newIndex}].ImportInvoiceNumber"
                      data-valmsg-replace="true"></span>
            </td>
            <td>
                <input type="text"
                       name="ExportedProducts[${newIndex}].ProductName"
                       class="form-control bg-dark text-white"
                       ...
                       data-val-maxlength-max="255" />
                <span class="text-danger field-validation-valid"
                      data-valmsg-for="ExportedProducts[${newIndex}].ProductName"
                      data-valmsg-replace="true"></span>
            </td>
            <td>
                <input type="number"
                       name="ExportedProducts[${newIndex}].Quantity"
                       class="form-control bg-dark text-white"
                ...
                <span class="text-danger field-validation-valid"
                      data-valmsg-for="ExportedProducts[${newIndex}].Quantity"
                      data-valmsg-replace="true"></span>
            </td>
            <td>
                <input type="number"
                       step="0.01"
                       name="ExportedProducts[${newIndex}].UnitPrice"
                       class="form-control bg-dark text-white"
                ...
                <span class="text-danger field-validation-valid"
                      data-valmsg-for="ExportedProducts[${newIndex}].UnitPrice"
                      data-valmsg-replace="true"></span>
            </td>
            <td>
                <input type="text"
                       name="ExportedProducts[${newIndex}].CategoryName"
                       class="form-control bg-dark text-white"
                       ...
                       data-val-maxlength-max="255" />
                <span class="text-danger field-validation-valid"
                      data-valmsg-for="ExportedProducts[${newIndex}].CategoryName"
                      data-valmsg-replace="true"></span>
            </td>
            <td>
        ...
        $rows.each(function (index) {
            ...
            $(this).find('input, span[data-valmsg-for]').each(function () {
                if (this.tagName.toLowerCase() === 'input') {
                    ...
                    const newName = name.replace(/ExportedProducts\[\d+\]/, `ExportedProducts[${index}]`);
                    $(this).attr('name', newName);

                    // Optional: update id attribute if present
                    const id = $(this).attr('id');
                    if (id) {
                        const newId = id.replace(/ExportedProducts_\d+__/, `ExportedProducts_${index}__`);
                        $(this).attr('id', newId);
                    }
                }
                ...
                if ($(this).attr('data-valmsg-for')) {
                    ...
                    const newValmsgFor = valmsgFor.replace(/ExportedProducts\[\d+\]/, `ExportedProducts[${index}]`);
